...
export const getUnits = cache(async () => {
	...
	if (!userProgress?.courseId || !userId) return [];
...
export const getCourseProgress = cache(async () => {
	const { userId } = await auth();
	const userProgress = await getUserProgress();
	if (!userProgress?.courseId || !userId) return null;

	const unitsInActivCourse = await db.query.units.findMany({
		where: eq(units.courseId, userProgress.courseId),
		orderBy: (units, { asc }) => [asc(units.order)],
		with: {
			lessons: {
	...
	const firstUnCompletedLesson = unitsInActivCourse
...
export const getTop10Users = cache(async () => {
	...
	const users = await db.query.userProgress.findMany({
		orderBy: (userProgress, { desc }) => [desc(userProgress.hearts)],
		limit: 10,
		columns: {
			userId: true,
			userName: true,
